...
<head>
    ...
    <script language="javascript">
        function jianum()
        {
            ...
            document.getElementById("totelj").innerHTML=(parseInt(num)+1)*{{$data[0]->jinbijiage}};
            document.getElementById("totely").innerHTML=(parseInt(num)+1)*{{$data[0]->yinbijiage}};
        }
        function jiannum()
        {
            var num=document.getElementById("jine").value;
            if(num<=1)
            {
                ...
                document.getElementById("totelj").innerHTML={{$data[0]->jinbijiage}};
                document.getElementById("totely").innerHTML={{$data[0]->yinbijiage}};
            }
            else
            {
                document.getElementById("jine").value=parseInt(num)-1;
        ...
        function jisuan()
        {
            ...
            else
            {
                ...
                document.getElementById("totelj").innerHTML=parseInt(num)*{{$data[0]->jinbijiage}};
                document.getElementById("totely").innerHTML=parseInt(num)*{{$data[0]->yinbijiage}};
            }
        }
    </script>
</head>
    ...
    <div class="content">
        ...
        <div class="renzheng">
            <table width="100%" border="0" style="text-align:center" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    <td><a href="{{URL('shop/shop-detail/'.$id)}}">{{$data[0]->title}}</a></td>
                    <td><img src="{{$data[0]->pic}}" width="100" height="100"  style="margin-top: 10px;" /></td>
                    <td>{{$data[0]->jinbijiage}}</td>
                    <td>{{$data[0]->yinbijiage}}</td>
                    <td width="310">
                        <div class="gentou">
                            <span></span>
                    ...
                    <td width="100"><font color="#FF0000" size="+2" id="totelj">{{$jine*$data[0]->jinbijiage}}</font></td>
                    <td width="100"><font color="#FF0000" size="+2" id="totely">{{$jine*$data[0]->yinbijiage}}</font></td>
                </tr>
            </table>
        </div>
        <div class="renzheng">
            <form name="form1" class="form" action="{{URL('shop/do/shoporder/'.$id)}}" method="post">
                <table width="100%" border="0" style="text-align:center" cellpadding="0" cellspacing="0">
                    <tr>
                        <td colspan="4">
