...
<div id="login">
    <div class="login">
        ...
        <div class="con">
            ...
            <form name="loginform" id="loginform" class="form" method="post" action="{{ URL('checklogin') }}">
                ...
                <li><span>验证码：</span>
                    <input type="text" nullmsg="请输入验证码！" errormsg="请输入验证码！" datatype="*" name="code" class="code" value="请输入验证码" onfocus="if(this.value=='请输入验证码'){this.value='';}" onblur="if(this.value==''){this.value='请输入验证码';}" ajaxurl="{{ URL('checkcode') }}" />
                    <img class="imgcode" title="点击刷新" src="{{ URL('codenum') }}" onclick="this.src='codenum?'+Math.random();">
                </li>
        ...
        @elseif($type=2)
            <div class="mess">
                <div class='mes'>
                    <h1>{{ $msg }}</h1>
                    <span id='nuber'>2</span> 秒钟后自动进入会员中心... <br><br>如果没有进入，请点击这里  <a href="{{URL('/')}}">会员中心</a>。
                    <script language="JavaScript">goto('{{ $url }}');</script>
                </div>
            </div>
        @elseif($type=5)
            <div class="mess">
                <div class='mes'>
                    <h1>{{ $msg }}</h1>
                    <span id='nuber'>2</span> 秒钟后自动返回登录... <br><br>如果没有自动返回，请点击这里  <a href="javascript:;" onclick="history.go(-1)">返回上一页</a>。
                    <script language="JavaScript">goto('{{ $url }}');</script>
                </div>
            </div>
        @elseif($type=6)
